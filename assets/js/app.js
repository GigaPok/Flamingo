...
requirejs(
  ["app/loading", "matter", "jquery", "datgui", "three", "gsap"],
  function (loading) {
    var isMobile = $("html").hasClass("mobile");
    var player = null;

    player = document.createElement("audio");
    player.src = "assets/audio/music2.mp3";
    player.preload = "auto";
    player.load();
    ...
    document
      ...
      .insertBefore(renderer.domElement, document.querySelectorAll("nav")[0]);
    ...
    if (location.search.split("?")[1] == "dev")
      engine.render.canvas.style.display = "block";
    ...
    Events.on(engine, "tick", function (event) {
      ...
      engine.world.gravity.y =
        params.y * Math.sin((20 * event.timestamp) / (Math.PI * params.timing));
      engine.world.gravity.x =
        params.x *
          Math.sin(
            (20 * event.timestamp) / 2 / (Math.PI * params.timing) -
              (1 * Math.PI) / 1
          ) -
        params.xOffset;
      obj.rotation.x =
        params.rock *
        Math.sin((20 * event.timestamp) / 2 / (Math.PI * params.timing));

      if (!mesh) return;

      for (var i = 0; i < mesh.skeleton.bones.length - 1; i++) {
        mesh.skeleton.bones[i].position.x =
          (points[i].position.x - 400) / params.scale;
        mesh.skeleton.bones[i].position.y =
          (360 - points[i].position.y) / params.scale;
        mesh.skeleton.bones[i].rotation.z = getAngle(
          rotPoints[i][0],
          rotPoints[i][1]
        );
      }
      ...
      if (params.pause) {
        ...
        params.x +=
          (0.5 *
            Math.sin((20 * engine.timing.timestamp) / (Math.PI * 17 * 1000)) +
            1.0 -
            params.x) /
          20;
        params.y +=
          (0.5 *
            Math.sin((20 * engine.timing.timestamp) / (Math.PI * 42 * 1000)) +
            1.25 -
            params.y) /
          20;
        params.rock +=
          (0.1 *
            Math.sin((20 * engine.timing.timestamp) / (Math.PI * 30 * 1000)) +
            0.2 -
            params.rock) /
          20;
      }
    ...
    var pauseMusic = function () {
      ...
      TweenLite.to($(this).find(".button"), 0.5, {
      ...
      control.pointA = {
        x: head.bodies[4].position.x + 1,
        y: head.bodies[4].position.y + 1,
      };
      ...
      controlNose.pointA = {
        x: head.bodies[5].position.x + 1,
        y: head.bodies[5].position.y + 1,
      };
      ...
      tl.add([
        TweenLite.set(dummy, {
          x: control.pointA.x + 1,
          y: control.pointA.y + 1,
        }),
        TweenLite.set(dummy, {
          rotationX: controlNose.pointA.x + 1,
          rotationY: controlNose.pointA.y + 1,
        }),

        TweenLite.to(dummy, 3, {
          x: targetX + 200,
          y: targetY - 180,
          ease: Power1.easeInOut,
          delay: 2.0,
        ...
        TweenLite.to(dummy, 3, {
          rotationX: targetX - 40,
          rotationY: targetY - 120,
          ease: Power1.easeInOut,
          delay: 2.0,
        }),
      ]);
      tl.add([
        TweenLite.to(dummy, 1, {
          x: "+=40",
          y: "-=40",
          ease: Power1.easeInOut,
        }),
      ...
      tl.add([
        TweenLite.to(dummy, 0.2, {
          x: targetX + 180,
          y: targetY - 180,
          ease: Power1.easeIn,
        }),
        TweenLite.to(dummy, 0.2, {
          rotationX: targetX - 60,
          rotationY: targetY - 120,
          ease: Power1.easeIn,
        }),
      ...
      tl.addCallback(function () {
        ...
        TweenLite.set($(".mute-prompt .button"), {
          boxShadow: "0px 0px 0px 0pt rgba(75, 52, 10, 0.3)",
        });
        TweenLite.to($(".mute-prompt .button"), 0.5, {
          boxShadow: "0px 0px 0px 10pt rgba(75, 52, 10, 0)",
          ease: Power1.easeOut,
        });
      }, 6.2);
    ...
    var shareThings = function () {
      ...
      TweenLite.set($(".share-prompt .button"), {
        boxShadow: "0px 0px 0px 0pt rgba(75, 52, 10, 0.3)",
      });
      TweenLite.to($(".share-prompt .button"), 0.5, {
        boxShadow: "0px 0px 0px 10pt rgba(75, 52, 10, 0)",
        ease: Power1.easeOut,
      });
      ...
      TweenMax.staggerTo(
        $(".network"),
        0.3,
        { x: 0, rotation: 0, opacity: 1, ease: Back.easeOut },
        0.05
      );
      ...
      clearShare = function () {
        ...
        TweenMax.staggerTo(
          $(".network"),
          0.15,
          { x: 10, opacity: 0, ease: Power1.easeOut },
          0.05
        );
      };
      setTimeout(function () {
    ...
    var updateMouseConstraint = function (e) {
      ...
      control.pointA.x =
        ((e.clientX - WIDTH / 2) / (HEIGHT / 0.75)) * 800 + 400;
    ...
    var onDocumentMouseDown = function (e) {
      ...
      control.angleB = 0;
      control.pointB = {
        x: control.pointA.x - control.bodyB.position.x,
        y: control.pointA.y - control.bodyB.position.y,
      };
